...
class Project {
    ...
    static save() {
        ...
        const updated = function (path) {
            CURRENT_PROJECT_PATH = path;
            Project.setProjectChanged(false);
            Project.updateRecentProjects(path);            
        };

        let path = FileSystem.saveProject(Project.getData(), CURRENT_PROJECT_PATH, updated);
    ...
    static saveAs() {
        FileSystem.saveProject(Project.getData(), "", (path) => {
            CURRENT_PROJECT_PATH = path;
            Project.setProjectChanged(false);
            Project.updateRecentProjects(path);
        });
    ...
    static load(pathToLoad = "") {
        Project.saveChanges(() => {
            FileSystem.loadProject(pathToLoad)
            .then(({ path, data }) => {
                if (data) {
                    Project.stopObserv();
